...
win_icon = PhotoImage(file='P:\Docs\VS code\Python\mini_project\icon\win_icon.png')
root.iconphoto(True, win_icon)

send_btn = PhotoImage(file='P:\Docs\VS code\Python\mini_project\icon\send_button.png')
files_btn = PhotoImage(file='P:\Docs\VS code\Python\mini_project\icon\\file.png')
...
def send():
    ...
    #
    time.sleep(10) # time selay for 10 seconds
    sbj = "Images you may like"
    msge = "Greetings Sir\n\nThis is an automated mail.\nYou bought some images from us, we do have many more images you can check them out.\nHere are some of our top rated images."
...
btn = Button(root, image=send_btn, padx=15, command=send, relief=FLAT) # 'relief' style of the button

label_email.place(relx=0.25, rely=0.02) # value of relx and rely should be between 0 and 1
...
label_message.place(relx=0.5, rely=0.45, anchor=CENTER) # 'anchor' it sets the text
...
# 2) then type 'pyinstaller --onefile --icon=email.ico -w new.py'
#    --icon=email.ico, if you want to use icon for the exe file
#    -w to disable the window when we open the .exe file
#    new.py is the name of the file(code)